...
class Visitor2(MiniJavaVisitor):
    ...
    def visitGoal(self, ctx):
        ...
        ctx.main_class_decl.accept(self)
        #return self.visitChildren(ctx)
    ...
    def visitMainclass(self, ctx):
        ...
        ctx.main_class_stmt.accept(self)
    ...
    def visitDec_class(self, ctx):
        ...
        try:
            ctx.class_Decl_var.accept(self)
        except:
            pass
        try:
            ctx.class_meth_decl.accept(self)
        except:
            pass
    ...
    def visitDec_method(self, ctx):
        ...
        method = Scope(self.escopos.get(classe_atual))

        #Checa o tipo de retorno da definicao
        retType = ctx.meth_mtype.accept(self)
        retType = retType.upper()
        list = ctx.Identifier()
        name = list[0]
        name = name.getText()
        method.insert(VariableEntry(retType, name))

        if retType != 'INT' and retType != 'INT_ARRAY' and retType != 'BOOLEAN' and (not retType in self.classes):
            lista = ctx.children
            self.erro_generico_detectado("Classe "+retType+" nao encontrada DecMethod", \
                                         lista[0].getSymbol())
            sys.exit()

        #Pega e checa parametros
        try:
            ctx.tipoP1.accept(self)
        except:
            pass
        try:
            for i in ctx.listaTipoPs:
                i.accept(self)
        except:
            pass
        ...
        '''
        print(self.isCompatible(retType, ctx.exp_retorno.accept(self)))
    ...
    def visitMtype(self, ctx):
        ...
        if type != 'INT' and type != 'INT_ARRAY' and type != 'BOOLEAN' and (not type in self.classes):
            lista = ctx.children
            self.erro_generico_detectado("Classe nao encontrada varDeclaration " + type, \
                                         lista[0].getSymbol())
            sys.exit()
        return type
    ...
    def visitState_if(self, ctx):
        ...
        res = ctx.expIf.accept(self)
        if res != 'BOOLEAN':
            self.erro_generico_detectado("Funcao do IF deve ser do tipo BOOLEAN", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
        ctx.statementIF1.accept(self)
        ctx.statementIF2.accept(self)
        return self.visitChildren(ctx)
    ...
    def visitState_while(self, ctx):
        # Checa a condicao do tipo
        res = ctx.expWhile.accept(self)
        if res != 'BOOLEAN':
            self.erro_generico_detectado("Funcao do While deve ser do tipo BOOLEAN", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
        ctx.statementWhile.accept(self)
        return None


    # Visit a parse tree produced by MiniJavaParser#state_print.
    def visitState_print(self, ctx):
        argType = ctx.expPrint.accept(self)
        if argType != 'INT' and argType != 'BOOLEAN':
            lista = ctx.children
            self.erro_generico_detectado("Funcao print so aceita INT ou BOOLEAN", \
                                        lista[0].getSymbol())
            sys.exit()
        return None
    ...
    def visitState_assign(self, ctx):
        ...
        #Arrumar ainda
        '''
        rtype = ctx.expression()
        print(rtype.getText())

        #Checa os tipos
        if not self.isCompatible(ltype, rtype):
            self.erro_generico_detectado("Variavel " + varName + ' tipos incompativeis!', \
                                         ctx.Identifier().getSymbol())
            sys.exit()
        '''
    ...
    def visitState_array_assign(self, ctx):
        ...
        res = ctx.expArrayAssign1.accept(self)
        if res != 'INT':
            self.erro_generico_detectado("O indice deve sempre ser do tipo INT", \
                                         ctx.Identifier().getSymbol())
            sys.exit()

        #Checa o tipo do array
        arrayID = ctx.Identifier().getText()
        scopo = self.escopos.get(classe_atual)
        if self.lookupField(scopo, arrayID) == None:
            self.erro_generico_detectado("Variavel "+arrayID+" nao foi declarada", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
        if self.lookupField(scopo, arrayID) != 'INT_ARRAY':
            self.erro_generico_detectado("Array com id " + arrayID + " diferente de INT_ARRAY", \
                                         ctx.Identifier().getSymbol())
            sys.exit()

        #Checa tipo da expressao

        rest1 = ctx.expArrayAssign2.accept(self)
        if rest1 != 'INT':
            self.erro_generico_detectado("Arrays pode somente valores do tipo INT", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_this.
    def visitExpr_this(self, ctx):
        scopo = self.classes.get(classe_atual)
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_op_and.
    def visitExpr_op_and(self, ctx):
        ltype = ctx.exp1.accept(self)
        rtype = ctx.exp2.accept(self)
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_op_less.
    def visitExpr_op_less(self, ctx):
        ltype = ctx.exp3.accept(self)
        rtype = ctx.exp4.accept(self)
        if ltype != 'INT' or rtype != 'INT':
            lista = ctx.children
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_op_plus.
    def visitExpr_op_plus(self, ctx):
        ltype = ctx.exp5.accept(self)
        rtype = ctx.exp6.accept(self)
        if ltype != 'INT' or rtype != 'INT':
            self.erro_generico_detectado("Ambos os lados da expressao + devem ser do tipo INT", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_op_minus.
    def visitExpr_op_minus(self, ctx):
        ltype = ctx.exp7.accept(self)
        rtype = ctx.exp8.accept(self)
        if ltype != 'INT' or rtype != 'INT':
            self.erro_generico_detectado("Ambos os lados da expressao - devem ser do tipo INT", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_op_multi.
    def visitExpr_op_multi(self, ctx):
        ltype = ctx.exp9.accept(self)
        rtype = ctx.exp10.accept(self)
        if ltype != 'INT' or rtype != 'INT':
            self.erro_generico_detectado("Ambos os lados da expressao * devem ser do tipo INT", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_array.
    def visitExpr_array(self, ctx):
        arrayType = ctx.expArray1.accept(self)
        if arrayType != 'INT_ARRAY':
            self.erro_generico_detectado("Matrizes so podem conter valores do tipo INT", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
        indexType = ctx.expArray2.accept(self)
        if indexType != 'INT':
            self.erro_generico_detectado("Indice do array so pode ser do tipo INT", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_length.
    def visitExpr_length(self, ctx):
        arrayType = ctx.expLegth.accept(self)
        if arrayType != 'INT_ARRAY':
    ...
    def visitExpr_method_calling(self, ctx):
        ...
        global classe_atual
        scope = self.escopos.get(classe_atual)
        className = ctx.exp_chamador.accept(self)
        ...
        if className == None or (not className in self.classes):
            self.erro_generico_detectado("Chamada para uma classe inexistente!!", \
                                         ctx.Identifier().getSymbol())
            sys.exit()
        ...
        if methodInfo == None:
            self.erro_generico_detectado("Class "+className+" nao possui o metodo da chamada", \
                                         ctx.Identifier().getSymbol())
        '''
        #Checa os parametros
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_int_array.
    def visitExpr_int_array(self, ctx):
        indexType = ctx.exp12.accept(self)
        if indexType != 'INT':
            self.erro_generico_detectado("O tamanho do Array deve ser expressao do tipo INT", ctx.Identifier().getSymbol())
            sys.exit()
        return "INT_ARRAY"
    ...
    def visitExpr_new_array(self, ctx):
        ...
        if not className in self.classes:
            self.erro_generico_detectado("Tentando instanciar um objeto de classe inexistente",
                                         ctx.Identifier().getSymbol())
            sys.exit()
        return className
    ...
    # Visit a parse tree produced by MiniJavaParser#expr_not.
    def visitExpr_not(self, ctx):
        return ctx.exp_not.accept(self)
